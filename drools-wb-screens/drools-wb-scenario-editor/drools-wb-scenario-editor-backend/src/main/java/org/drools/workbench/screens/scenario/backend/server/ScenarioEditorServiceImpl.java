...
package org.drools.workbench.screens.scenario.backend.server;
...
import org.drools.workbench.screens.scenario.backend.server.util.ScenarioPersistence;
...
@Service
@ApplicationScoped
public class ScenarioEditorServiceImpl
        extends KieService<ScenarioEditorContent>
        implements ScenarioEditorService {
    ...
    @Inject
    private SaveAndRenameServiceImpl<ScenarioModel, Metadata> saveAndRenameService;

    private SafeSessionInfo safeSessionInfo;

    public ScenarioEditorServiceImpl() {
    }

    @Inject
    public ScenarioEditorServiceImpl(final SessionInfo sessionInfo) {
        ...
        try {
            ...
            if (generate) {
                ...
                ioService.write(nioPath,
                                ScenarioPersistence.getInstance().marshal(content),
    ...
    @Override
    public ScenarioModel load(final Path path) {
        try {
            final String content = ioService.readAllString(Paths.convert(path));

            return ScenarioPersistence.getInstance().unmarshal(content);
        } catch (Exception e) {
            throw ExceptionUtilities.handleException(e);
        }
    }
    ...
    @Override
    public ScenarioEditorContent loadContent(final Path path) {
        return super.loadContent(path);
    }

    @Override
    protected ScenarioEditorContent constructContent(Path path,
                                                    Overview overview) {
        //De-serialize model
        final ScenarioModel model = load(path);
        ...
        return new ScenarioEditorContent(model,
                                        overview,
    ...
    public String toSource(final Path path,
        ...
        return sourceServices.getServiceFor(Paths.convert(path)).getSource(Paths.convert(path),
                                                                           ScenarioPersistence.getInstance().marshal(model));
    }
